...
exports.registerUser = catchAsyncErrors(async (req, res, next) => {
  ...
  const { companyname, cemail, cin, city, postalcode } = req.body;
...
exports.loginUser = catchAsyncErrors(async (req, res, next) => {
  ...
  if (!cemail || !otp) {
    return next(new ErrorHander("Please enter email and otp", 400));
  }

  const user = await User.findOne({ cemail }).select("+password");
...
exports.updateProfile = catchAsyncErrors(async (req, res, next) => {
  const newUserData = {
    ...
    cemail: req.body.lastname,
    cin: req.body.email,
    postalcode: req.body.hnumber,
